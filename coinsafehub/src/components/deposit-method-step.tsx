...
export function DepositMethodStep({
  amount,
  selectedMethod,
  onMethodSelect,
  onBack,
  ...
  const calculateNetAmount = (method: PaymentMethod) => {
    return numAmount - calculateFee(method)
  }

  const handleMethodClick = (method: PaymentMethod) => {
    if (numAmount < method.minAmount) {
      setError(`Minimum deposit for ${method.name} is $${method.minAmount}`)
      return
    }
    
    if (numAmount > method.maxAmount) {
      setError(`Maximum deposit for ${method.name} is $${method.maxAmount.toLocaleString()}`)
      return
    }
  ...
  return (
    <div className="max-w-4xl mx-auto w-full space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Depositing <span className="font-semibold text-foreground">${numAmount.toFixed(2)}</span>
          </p>
        </div>
      </div>
      ...
      <div className="p-4 bg-muted rounded-lg space-y-2 text-sm">
        <p className="font-medium">💡 Tips for choosing a payment method:</p>
        <ul className="space-y-1 text-muted-foreground ml-4">
          <li>• Choose crypto for faster processing (5-30 minutes)</li>
          <li>• Bank wire is best for large amounts ($500+)</li>
          <li>• Stablecoins (USDT, USDC) avoid price volatility</li>
          <li>• Lower fees mean more money in your account</li>
        </ul>
      </div>
    </div>
  )
